...
masterPasswordCheck = b'SUFPfDbpAkOYCzOg6QcxgO9h3niXwnV1vJqQ7bSBMUU='
...
while True:
    if not loggedIn:
        ...
        if encrypter.encrypt("Gyr0") != masterPassword:
    ...
    if choice == 1:
        for entry in passwords:
            if app in encrypter.decrypt(entry[0]):
                print("\n----------------------------------------------")
                print(f"Applciation: {encrypter.decrypt(entry[0])}")
